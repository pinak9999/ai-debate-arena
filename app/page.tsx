...
export default function HeroSection({ onStart, mode, setMode, selectedLang, setSelectedLang, disabled }: HeroSectionProps) {
  ...
  return (
    <motion.div
      className="relative min-h-screen w-full flex flex-col items-center justify-between px-4 pt-8 pb-6 z-10 overflow-hidden bg-[#08090c]"
    ...
    >
      ...
      <div className="w-full max-w-2xl flex flex-col items-center my-auto z-10 pt-4">
        
        {/* ── TOP CONTROLS: Game Mode & Language Selector ── */}
        <motion.div variants={itemVariants} className="w-full flex flex-col sm:flex-row items-center justify-center gap-4 mb-6">
          <div className="flex flex-col items-center">
            <p className="text-gray-400 text-[10px] mb-1.5 uppercase tracking-widest font-bold">Select Game Mode</p>
            <ModeToggle mode={mode} setMode={setMode} disabled={disabled} />
          </div>

          <div className="flex flex-col items-center">
            <p className="text-gray-400 text-[10px] mb-1.5 uppercase tracking-widest font-bold">Select Language</p>
            <select 
              value={selectedLang} 
              onChange={(e) => setSelectedLang(e.target.value as DebateLanguage)}
              className="bg-black/40 border border-white/10 text-white text-xs uppercase tracking-widest px-5 py-2 rounded-full cursor-pointer hover:border-blue-500/50 transition-all outline-none"
            >
        ...
        {/* ── Eyebrow Badge ── */}
        <motion.div variants={itemVariants} className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-white/10 bg-white/[0.04] backdrop-blur-md mb-5">
        ...
        <motion.div variants={itemVariants} className="text-center mb-5">
          <h1 className="font-['Anton','Arial_Narrow',sans-serif] font-normal uppercase leading-[0.86] tracking-[0.01em] text-[clamp(48px,11vw,110px)]">
          ...
          <p className="mt-3 font-mono text-[11px] tracking-[0.22em] uppercase text-[#5b616e]">
            <strong className="text-[#9ba1ad] font-semibold">Two Agents</strong>
            <span className="mx-2 text-[#333844]">/</span>
        ...
        <motion.div variants={itemVariants} className="flex items-center justify-center my-3 w-full">
          <div className="flex flex-col items-center gap-1 w-[100px]">
            <div className="w-[44px] h-[44px] rounded-full flex items-center justify-center font-['Anton'] text-lg border-2 border-[#3b7bff] text-[#3b7bff] shadow-[0_0_15px_rgba(59,123,255,0.3)]">A</div>
            <span className="font-mono text-[9px] tracking-[0.14em] uppercase text-[#5b616e]">Agent <strong className="text-[#9ba1ad]">Blue</strong></span>
          </div>
          <div className="flex-1 h-[1px] bg-gradient-to-r from-transparent to-white/10" />
          <div className="relative w-[48px] h-[48px] mx-[-4px] flex items-center justify-center flex-shrink-0">
            ...
            <span className="relative font-['Anton'] text-sm text-[#eef0f3] bg-[#08090c] w-[38px] h-[38px] rounded-full flex items-center justify-center">VS</span>
          </div>
          <div className="flex-1 h-[1px] bg-gradient-to-l from-transparent to-white/10" />
          <div className="flex flex-col items-center gap-1 w-[100px]">
            <div className="w-[44px] h-[44px] rounded-full flex items-center justify-center font-['Anton'] text-lg border-2 border-[#ff3d5a] text-[#ff3d5a] shadow-[0_0_15px_rgba(255,61,90,0.3)]">B</div>
            <span className="font-mono text-[9px] tracking-[0.14em] uppercase text-[#5b616e]">Agent <strong className="text-[#9ba1ad]">Red</strong></span>
          </div>
        </motion.div>

        {/* ── Mode Selection Buttons ── */}
        <motion.div variants={itemVariants} className="flex flex-wrap justify-center gap-2 my-4">
          <button
            onClick={() => { setSubject('topic'); setTopic(''); }}
            className={`flex items-center gap-2 px-4 py-2 rounded-full font-mono text-[10.5px] font-semibold tracking-[0.14em] uppercase border transition-all duration-200 cursor-pointer ${
              subject === 'topic' ? 'text-[#eef0f3] border-[var(--accent)] bg-[color-mix(in_srgb,var(--accent)_14%,transparent)] shadow-[0_0_18px_var(--accent-dim)]' : 'border-white/10 bg-white/[0.03] text-[#5b616e] hover:bg-white/[0.06] hover:text-[#9ba1ad]'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-full font-mono text-[10.5px] font-semibold tracking-[0.14em] uppercase border transition-all duration-200 cursor-pointer ${
              isStock ? 'text-[#eef0f3] border-[var(--accent)] bg-[color-mix(in_srgb,var(--accent)_14%,transparent)] shadow-[0_0_18px_var(--accent-dim)]' : 'border-white/10 bg-white/[0.03] text-[#5b616e] hover:bg-white/[0.06] hover:text-[#9ba1ad]'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-full font-mono text-[10.5px] font-semibold tracking-[0.14em] uppercase border transition-all duration-200 cursor-pointer ${
              isPersonality ? 'text-[#eef0f3] border-[var(--accent)] bg-[color-mix(in_srgb,var(--accent)_14%,transparent)] shadow-[0_0_18px_var(--accent-dim)]' : 'border-white/10 bg-white/[0.03] text-[#5b616e] hover:bg-white/[0.06] hover:text-[#9ba1ad]'
            }`}
          >
        ...
        <motion.div variants={itemVariants} className="w-full">
          <div className="relative bg-gradient-to-b from-white/[0.05] to-white/[0.02] border border-white/10 rounded-[20px] p-6 md:p-7 backdrop-blur-xl shadow-[0_30px_80px_-30px_rgba(0,0,0,0.7)]">
            
            {/* Field Label */}
            <div className="flex justify-between items-baseline mb-2">
              <span className="font-mono text-[10px] font-semibold tracking-[0.16em] uppercase text-[#9ba1ad]">
                {isStock ? '📈 Stock Ticker (NSE)' : isPersonality ? '🎭 Topic (Analyst vs Philosopher)' : 'Debate Topic'}
              </span>
              {!isStock && (
                <span className="font-mono text-[9px] text-[#5b616e]">⌘ + ↵ to start · {topic.length} chars</span>
              )}
            </div>
            ...
            <div className="relative mb-4">
              <textarea
                ...
                rows={isStock ? 1 : 3}
                className="w-full bg-[#040508]/70 border border-white/10 rounded-xl p-3.5 text-[#eef0f3] placeholder-[#5b616e] text-sm md:text-[14px] resize-none outline-none transition-all duration-200 focus:border-[var(--accent)] focus:shadow-[0_0_0_3px_var(--accent-dim)] font-sans"
              />
            </div>

            {/* Chips / Quick Examples */}
            <div className="font-mono text-[10px] font-semibold tracking-[0.16em] uppercase text-[#9ba1ad] mb-2">
            ...
            <div className="flex flex-wrap gap-1.5 mb-5">
              {examples.map((t) => (
                <button
                  key={t}
                  onClick={() => setTopic(t)}
                  className="font-sans text-[11px] px-3 py-1.5 rounded-lg border border-white/10 bg-white/[0.03] text-[#9ba1ad] hover:bg-white/[0.07] hover:text-[#eef0f3] hover:border-white/20 transition-all cursor-pointer"
                >
                  {t}
                </button>
              ))}
            ...
            {/* Rounds Selector */}
            <div className="font-mono text-[10px] font-semibold tracking-[0.16em] uppercase text-[#9ba1ad] mb-2">Rounds</div>
              ...
              {([3, 5, 7] as const).map((r) => {
                ...
                return (
                  <button
                    key={r}
                    onClick={() => setRounds(r)}
                    className={`flex-1 py-3 rounded-xl border text-center cursor-pointer font-mono text-[12px] font-semibold tracking-[0.040em] transition-all duration-200 ${
                      active 
                  ...
                  >
                    <span className="block font-['Anton'] text-lg text-inherit mb-0">{r}</span>
                    Rounds
                  </button>
                );
              })}
            ...
            <motion.button
              ...
              className={`w-full py-3.5 rounded-xl font-mono font-bold text-xs tracking-[0.18em] uppercase flex items-center justify-center gap-2.5 text-[#08090c] transition-all duration-300 ${
      ...
      {/* ── Marquee Ticker Tape ── */}
      <motion.div variants={itemVariants} className="w-full mt-6 overflow-hidden border-t border-b border-white/10 py-2.5 relative z-10 [mask-image:linear-gradient(90deg,transparent,#000_8%,#000_92%,transparent)]">
