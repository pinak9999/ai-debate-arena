...
// Set maximum execution time for the serverless function (60 seconds)
export const maxDuration = 60;
// Force dynamic rendering (disables static caching for this API route)
export const dynamic = 'force-dynamic';

// ─── UTILITY FUNCTIONS ───

// Safely parses JSON by stripping out markdown formatting (like ```json)
...
// Removes fake citations (e.g., "Source: ...") generated by the AI
function stripFakeCitations(text: string): string {
  return text
    .replace(/\(?\s*(Source|Reference)\s*[:：].*?(\)|(?=\n)|$)/gi, '')
    .replace(/\s{2,}/g, ' ')
    .trim();
}

// Removes AI meta-commentary or apologies (e.g., "My previous statement...")
function stripMetaCommentary(text: string): string {
  return text
    .replace(/my (previous )?statement[^.!?]*(confused|criticism|review|again)[^.!?]*[.!?]/gi, '')
    .replace(/[^.!?]*(critic|reviewer|review|in response to criticism)[^.!?]*[.!?]/gi, '')
    .replace(/\s{2,}/g, ' ')
    .trim();
}

// Converts plain text into a ReadableStream to stream the response back to the client
...
// Generates a short 3-5 word search query from a long text using Gemini Flash Lite
async function generateSearchQuery(text: string): Promise<string> {
  try {
    const { text: query } = await generateText({
      model: google('gemini-3.5-flash-lite'), // Fast model for quick query generation
    ...
    // Fallback: Just take the first 5 words of the statement if the AI fails
...
// Helper to search Wikipedia via API
...
// Fetches a short snippet summary from Wikipedia for factual grounding
async function fetchWikiSnippet(query: string): Promise<{ title: string; snippet: string; url: string | null } | null> {
  let topResult = await searchWiki('en', query); // Defaults to English search first
  let lang: 'hi' | 'en' = 'en';
...
// Combines search query generation and Wikipedia fetching
...
// Queries the Tavily API for live internet search results
async function searchTavily(query: string): Promise<{ answer: string | null; sources: TavilySource[] } | null> {
  const apiKey = process.env.TAVILY_API_KEY;
  if (!apiKey || !query?.trim()) return null;
  try {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 4000); // 4-second timeout to prevent stalling
...
// FIX: Handles Tavily search and provides a safe fallback if it crashes
async function groundWithTavily(text: string): Promise<{ snippet: string; sources: TavilySource[] } | null> {
  try {
    ...
    // If no data is found from Tavily, return a generic logical fallback
...
// Type guard to validate audience score object
...
// Adjusts AI behavior (prompt instructions) dynamically based on live audience voting
...
// Helper to keep scores within a valid min/max range
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!process.env.GOOGLE_GENERATIVE_AI_API_KEY) {
      return NextResponse.json({ error: 'FATAL ERROR: GOOGLE_GENERATIVE_AI_API_KEY Environment Variable is missing!' }, { status: 500 });
    }
    ...
    try {
      ...
      return NextResponse.json({ error: 'FATAL ERROR: Invalid or empty JSON body received from frontend.' }, { status: 400 });
    }

    // 🔥 THE MASTER TPM FIX: Truncate large context strings to save Tokens Per Minute (TPM) limits
    ...
    // ─────────────────────────────────────────────────────────────────
    // ROUTE 1: FETCH LIVE STOCK DATA (Yahoo Finance)
    // ─────────────────────────────────────────────────────────────────
    if (body.type === 'stock_data') {
      ...
      try {
        ...
        // Parsing and formatting Yahoo Finance response data
    ...
    // ─────────────────────────────────────────────────────────────────
    // ROUTE 2: DEBATE TURN GENERATION (The Core AI Generation)
    // ─────────────────────────────────────────────────────────────────
    if (body.type === 'debate_turn') {
      const { topic, round, totalRounds, speaker, history = [], mode = 'topic', stockContext, audienceScore, language = 'English' } = body;
      if (!topic || !speaker || !round) return NextResponse.json({ error: 'Missing required fields' }, { status: 400 });

      // Identify the debate mode
      ...
      // RAG Grounding Logic: Inject live facts based on debate mode
      let groundingBlock = '';
      if (isDocumentMode) {
...
3. HUMAN LENGTH: Keep responses punchy and focused (60-80 words). Speak like a seasoned human orator in native script.
      ...
      const langInstruction = `CRITICAL RULE: You MUST write your entire response STRICTLY in ${language.toUpperCase()} using its NATIVE SCRIPT ONLY.`;

      // Assign specific prompts based on the current round and mode
      ...
      // Final Assembly of the System Prompt
...
Respond directly and STRICTLY in ${language} native script (NO ENGLISH LETTERS) without formal greetings and avoid robotic connector words.` 
      ...
      // Trigger Gemini API to generate the debate text
      const { text: rawOutput } = await generateText({
        model: google('gemini-3.5-flash-lite'), // Fast model applied
        temperature: 0.7,
        system: systemPrompt,
      ...
      // Fallback mechanism to intercept AI safety refusals ("I am sorry", "I cannot fulfill")
      if (
        ...
        safeOutput = "The opponent's logic completely falls apart under scrutiny. Instead of addressing the core issue, they rely on flawed assumptions. The empirical evidence clearly dictates that my stance is the only rationally sound conclusion in this scenario.";
      }

      // Clean the AI output from fake citations and meta-commentary
      let cleanOutput = stripMetaCommentary(stripFakeCitations(safeOutput));

      // 🔥 THE ULTIMATE FIX: If cleaning results in an empty string, provide a generic debate claim
      if (!cleanOutput || cleanOutput.trim() === '') {
        console.warn("⚠️ AI returned empty string after cleaning! Triggering blank fallback.");
        cleanOutput = "At this point, the opponent's claims have no solid basis. The available evidence and data clearly support my stance, rendering their claims hollow.";
      }
    ...
    // ─────────────────────────────────────────────────────────────────
    // ROUTE 3: INTER-ROUND JUDGE CRITIQUE (Quick Feedback)
    // ─────────────────────────────────────────────────────────────────
    if (body.type === 'judge_critique') {
      const { history = [], mode = 'topic', language = 'English' } = body;
    ...
    // ─────────────────────────────────────────────────────────────────
    // ROUTE 4: FINAL JUDGE VERDICT (End of Debate Evaluation)
    // ─────────────────────────────────────────────────────────────────
    if (body.type === 'judge_verdict') {
      const { topic, history = [], mode = 'topic', language = 'English', audienceScore } = body;
      const biasNote = mode === 'personality'
      ...
      // Evaluate historical transcript to aggregate penalties
...
Respond STRICTLY with a RAW JSON object. DO NOT wrap the JSON in markdown blocks (no \`\`\`json). DO NOT include any text before or after the JSON.
{
  ...
  "reasoning": "summary written STRICTLY in ${language} native script explaining the score differences."
      ...
      // Apply aggregated round penalties
    ...
    // ─────────────────────────────────────────────────────────────────
    // ROUTE 5: ROUND SCORE CALCULATION
    // ─────────────────────────────────────────────────────────────────
    if (body.type === 'round_score') {
      ...
      // Look for specific system penalties inside the turn messages
...
Respond STRICTLY with JSON ONLY. Do NOT wrap in \`\`\`json: {"pro": <number>, "opp": <number>}`;
    ...
    // ─────────────────────────────────────────────────────────────────
    // ROUTE 6: FALLACY & TONE CHECKER
    // ─────────────────────────────────────────────────────────────────
    if (body.type === 'fallacy_check') {
      const { text, topic, language = 'English' } = body;
